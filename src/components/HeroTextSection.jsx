  ...
  return (
    <section className="relative bg-black container-fluid topbanner overflow-hidden">
      <Container className="content-maxwidth hero-topbanner-content">
        <div>
          <div className="flex flex-wrap sm:flex-nowrap items-start gap-2 sm:gap-3 md:gap-4 mt-0.5 sm:mt-2">
            {/* Left: Headline */}
            <div className="w-full sm:flex-1 flex flex-col items-start text-left sm:w-auto">
              <h1 className="animated-gradient-text font-semibold leading-[1.25] tracking-tight mb-1.5 sm:mb-2 md:mb-4 lg:mb-6 text-[16px] sm:text-[24px] md:text-[32px] lg:text-[50px]">
                Alchemy for the
                <br />
                Intelligent Age
              </h1>
            ...
            <div className="w-full sm:flex-[1.5] flex flex-col items-start text-left mt-1.5 sm:mt-0 sm:w-auto">
              <p className="text-justify text-xs sm:text-base md:text-lg lg:text-xl font-semibold text-white leading-snug">
          ...
          <motion.button
            ...
            transition={{ duration: 0.2 }}
            onClick={() => window.open("https://calendly.com/", "_blank")}
            className="flex sm:hidden group mt-1.5 px-2 py-0.5 bg-orange-500 font-bold rounded-full transition-all duration-300 items-center gap-0.5 hover:shadow-lg hover:bg-white border border-orange-500 cursor-pointer text-[10px]"
          >
            <Calendar className="w-2 h-2 text-white group-hover:text-orange-500 transition-colors duration-300 flex-shrink-0" />
            <span className="relative z-10 font-semibold text-white group-hover:text-orange-500 transition-colors duration-300 leading-tight">
              Book a Free Consultation for 30 Minutes
            </span>
          </motion.button>
